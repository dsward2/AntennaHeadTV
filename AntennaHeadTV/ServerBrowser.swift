...
/// Finds AntennaHead Macs on the LAN via Bonjour, for `ConnectScreen`'s
/// server list. Browses the dedicated `_antennahead._tcp` type (see
...
/// `NSLocalNetworkUsageDescription` in Info.plist; tvOS shows its Local
/// Network permission prompt the first time `start()` runs. The manual
/// host:port field stays alongside the list for when discovery can't work —
/// multicast filtered, the Mac on another subnet, or permission declined.
///
